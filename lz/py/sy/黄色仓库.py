...
class Spider(Spider):
    ...
    def getDynamicHost(self):
        """动态获取主机地址"""
        try:
            return "http://6590ck.cc/"  # 直接使用已知有效地址
    ...
    def detailContent(self, array):
        ...
        try:
            ...
            # 尝试提取更多信息
            try:
                content_text = root('.stui-pannel-bd').text()
                if content_text:
                    vod["vod_content"] = content_text[:200] + "..."
                else:
                    vod["vod_content"] = title  # 简介为空时用标题替代
            except Exception as e:
                print(f"提取简介失败: {e}")
                vod["vod_content"] = title  # 异常时用标题替代
